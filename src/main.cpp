...
int main(int argc, char* argv[])
{
    ...
    Shader Phong;
    Phong.CreateFromFiles("../../../Shaders/phong_shdrML.vert", "../../../Shaders/phong_shdrML.frag");

    Light Sol;
    Sol.setShaderLight(Phong.GetShaderID());
    ...
    glm::mat4 projection = glm::perspective(glm::radians(45.0f), (GLfloat)mainWindow.getBufferWidth() / (GLfloat)mainWindow.getBufferHeight(), (float)p_near, (float)p_far);
    ...
    while (!mainWindow.getShouldClose())
    {
        ...
        Phong.UseShader();
        uniformModel = Phong.GetModelLocation();
        uniformProjection = Phong.GetProjectionLocation();
        uniformView = Phong.GetViewLocation();
        ...
        //Aplicar iluminacio del sol
        Sol.Iluminacio(Phong.GetShaderID());

        //Skybox
        Sky.drawSkybox(POLARY, projection, camera.calculateViewMatrix());
