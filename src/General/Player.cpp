...
Player::Player(std::string const& modelPath, std::string const& modelName, btDiscreteDynamicsWorld* dynamicsWorld, GLfloat sIntensity, GLfloat shine, bool addHitbox) : modelWheel(NULL) {
	modelChasis = new Model(sIntensity, shine);
	modelChasis->LoadModel(modelPath, modelName);
...
Player::Player(Model* m, btDiscreteDynamicsWorld* dynamicsWorld) : modelWheel(NULL)
{
...
void Player::CreateVehicle(btDiscreteDynamicsWorld* dynamicsWorld) {
	...
	for (int i = 0, numModels = modelChasis->meshList.size(); i < numModels; i++) {
		btConvexHullShape convexShape;

		//TODO: Adaptar a nuevo mesh [DONE?]
		Mesh* mesh = this->modelChasis->meshList[i];
		const std::vector<GLfloat>* vertices = mesh->GetVertices();
		for (int j = 0; j < vertices->size(); j += 8) {
			btVector3 v1((*vertices)[j], (*vertices)[j + 1], (*vertices)[j + 2]);
	...
	// All the wheel configuration assumes the vehicle is centered at the origin and a right handed coordinate system is used
	btVector3 wheelConnectionPoint(halfExtents.x(), halfExtents.y() + 0.2, halfExtents.z());
